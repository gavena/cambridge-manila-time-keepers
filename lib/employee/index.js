...
employees.findByUsernameAndSchedIdAndDelete = (username1, scheduleid ,callback) => {

      console.log("scope.form.findByUsernameAndSchedIdAndDelete SUCESSS ="+scheduleid);
    //Employee.findOneAndUpdate(details._id, details, (error, schedule) => {
        Employee.update(
          {"username" : username1} ,
                {$pull : {"schedule" : {"sched" : scheduleid } } },
           (error, doc)=> {
        if (error) {
            return callback(error);
        }
...
employees.findByUsernameAndSchedIdAndUpdate = (username1, scheduleid ,callback) => {

      console.log("scope.form.findByUsernameAndSchedIdAndDelete SUCESSS ="+scheduleid);
    //Employee.findOneAndUpdate(details._id, details, (error, schedule) => {
        Employee.update(
          {username : username1, "schedule.sched" : {$ne : scheduleid }} ,
                {$addToSet : {"schedule" : {'sched' : scheduleid }} }
                 ,
           (error, doc)=> {
        if (error) {
            return callback(error);
